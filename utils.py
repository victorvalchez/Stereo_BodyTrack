...
def estimate_3d_point_from_single_view(P, uv, depth=1.0):
    """
    Estimate the 3D point from a single camera view using a predefined depth.
    P: Camera projection matrix
    uv: 2D keypoint
    depth: Assumed depth value
    """
    # Convert 2D keypoint to homogeneous coordinates
    uv_homogeneous = np.array([uv[0], uv[1], 1.0])
    
    # Back-project the 2D point to 3D space
    # P_inv is the pseudo-inverse of the camera projection matrix
    P_inv = np.linalg.pinv(P)
    point_3d_homogeneous = P_inv @ uv_homogeneous
    
    # Scale the point by the assumed depth
